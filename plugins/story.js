...
Ktb.addCommand({pattern: 'story ?(.*)', fromMe: false, desc: "Downloads STORY from instagaram" , dontAddCommandList: true }, async (message, match) => {
  ...
  const data = await axios(`https://xteam.xyz/dl/igs?nama=${match[1]}&APIKEY=ab9942f95c09ca89`)
    ...
    if (type === 'image') return await message.sendMessage(Buffer.from(msg.data), MessageType.image, { caption: Config.ALL_CAPTION, quoted: message.data })

    if (type === 'video') return await message.sendMessage(Buffer.from(msg.data), MessageType.video, { caption: Config.ALL_CAPTION, quoted: message.data })
